import type { BoardObject, TextStyle } from '../../../shared/types.js';
...
export class TextEditor {
  ...
  private _buildTextToolbar(obj: BoardObject & { type: 'text' }): HTMLDivElement {
    ...
    const current: TextStyle = { ...defaults, ...(obj.style || {}) };
    const state = { ...current };
    const emit = (patch: Partial<TextStyle>): void => { this.callbacks.onTextStyleChange?.(obj.id, patch); };
    ...
    size.addEventListener('change', () => {
      state.size = size.value as TextStyle['size'];
  ...
  private _positionInput(input: HTMLTextAreaElement, obj: BoardObject): void {
  ...
  private _positionToolbar(toolbar: HTMLDivElement, obj: BoardObject): void {
    const { x: sx, y: sy } = this.camera.worldToScreen(obj.x, obj.y);
