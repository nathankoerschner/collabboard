import type { BoardObject, ToolName } from '../../../shared/types.js';
import { Camera } from './Camera.js';
import { Renderer, RevealState } from './Renderer.js';
import { InputHandler } from './InputHandler.js';
import { TextEditor } from './TextEditor.js';
import type { ObjectStore } from '../board/ObjectStore.js';
import type { CursorManager } from '../board/CursorManager.js';
...
export class Canvas {
  ...
  aiRevealMap = new Map<string, AIRevealEntry>();
  ...
  constructor(canvasEl: HTMLCanvasElement, objectStore: ObjectStore, cursorManager: CursorManager | null, callbacks: CanvasCallbacks = {}) {
    ...
    this.inputHandler = new InputHandler(canvasEl, this.camera, () => this.objectStore.getAll(), {
      ...
      onCreate: (type, x, y, w, h) => {
        const obj = this.objectStore.createObject(type as BoardObject['type'], x, y, w, h);
  ...
  private _pointInObjectAabb(wx: number, wy: number, obj: BoardObject): boolean {
  ...
  private _draw(): void {
    ...
    const selectedObjects = this.selectedIds.map((id) => byId.get(id)).filter((o): o is BoardObject => o != null);
  ...
  private _trackAIReveals(objects: BoardObject[]): void {
  ...
  private _getRevealState(obj: BoardObject): RevealState | null {
