...
export function inverseRotatePoint(px, py, cx, cy, angleDeg) {
...
export function pointInRotatedRect(px, py, obj) {
  const c = getObjectCenter(obj);
...
export function pointInObject(px, py, obj) {
...
export function objectContainsObject(container, child) {
...
export function hitTestConnector(px, py, connector, objectsById, tolerance = 8) {
...
export function getPortList(obj) {
  const x = obj.x;
  const y = obj.y;
  const w = obj.width;
  const h = obj.height;
...
export function getPortPosition(obj, portName) {
  const ports = getPortList(obj);
  const port = ports.find((p) => p.name === portName);
  return port ? { x: port.x, y: port.y } : null;
}

export function findClosestPort(obj, px, py) {
  const ports = getPortList(obj);
  let best = null;
...
export function getConnectorEndpoints(connector, objectsById) {
...
export function getFrameHitArea(px, py, frame) {
...
export function getRotationHandlePoint(bounds) {
  return {
