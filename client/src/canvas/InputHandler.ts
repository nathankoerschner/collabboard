import type { BoardObject, ConnectorObject, HandleName, Point, Rect, ToolName } from '../../../shared/types.js';
...
export interface InputHandlerCallbacks {
  ...
  onCreate?: (type: string, x: number, y: number, w: number, h: number) => BoardObject | undefined;
  ...
  onStartConnector?: (wx: number, wy: number) => ConnectorObject | undefined;
  onResolveAttach?: (wx: number, wy: number, connectorId: string) => { object: BoardObject; port: PortInfo } | null;
  onConnectorEndpoint?: (id: string, side: 'start' | 'end', payload: { objectId?: string; port?: string; point?: Point }) => void;
...
export class InputHandler {
  ...
  private _eventWorld(e: MouseEvent): { sx: number; sy: number; wx: number; wy: number } {
  ...
  private _createForTool(wx: number, wy: number): BoardObject | undefined {
  ...
  private _beginEmptyDrag(e: MouseEvent, sx: number, sy: number, wx: number, wy: number): void {
  ...
  private _handleResize(wx: number, wy: number): void {
    const dx = wx - this.dragStartX;
    const dy = wy - this.dragStartY;
