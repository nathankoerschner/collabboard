...
export class ObjectStore {
  ...
  moveObjects(ids, dx, dy) {
    ...
    this.doc.transact(() => {
      for (const id of moveSet) {
        const obj = this.getObject(id);
        if (!obj) continue;
        this._setObject({ ...obj, x: obj.x + dx, y: obj.y + dy });
      }
  ...
  rotateObjects(ids, deltaAngle, pivot = null) {
    if (!ids.length || deltaAngle === 0) return;

    const current = ids.map((id) => this.getObject(id)).filter(Boolean);
  ...
  updateColor(id, color) {
    ...
    if (!obj) return;
    this.doc.transact(() => {
      this._setObject({ ...obj, color });
    });
  }

  updateConnectorEndpoint(id, side, payload) {
    const obj = this.getObject(id);
    if (!obj || obj.type !== 'connector') return;
  ...
  deleteObjects(ids) {
    ...
    this.doc.transact(() => {
      ...
      const deleting = [...expanded].map((id) => this.getObject(id)).filter(Boolean);
  ...
  pasteSerialized(serializedObjects, placement = { x: 0, y: 0 }, relativeOffset = false) {
    ...
    for (const obj of clones) {
      ...
      if (obj.type === 'frame') {
        obj.children = (obj.children || []).map((id) => idMap.get(id)).filter(Boolean);
      }
      if (obj.parentFrameId) {
  ...
  _buildDefaultObject(id, type, x, y, width, height, extra) {
    const base = {
      id,
      type,
      x,
    ...
    if (type === 'sticky') {
      return { ...base, text: '', color: 'yellow', ...extra };
    }
    if (type === 'rectangle') {
      return { ...base, color: 'blue', strokeColor: 'gray', ...extra };
    }
    if (type === 'ellipse') {
      return { ...base, color: 'teal', strokeColor: 'gray', ...extra };
    }
    if (type === 'text') {
      return {
        ...base,
  ...
  _resolveConnectorPoint(connector, side) {
  ...
  _setObject(obj) {
    ...
    this.objectsMap.set(obj.id, yObj);
  }
  ...
  _removeFromZOrder(id) {
    const idx = this._zIndexOf(id);
    if (idx >= 0) this.zOrder.delete(idx, 1);
  }

  _getFrameDescendants(frameId) {
    const out = [];
    const walk = (id) => {
  ...
  _removeFrameChild(frameId, childId) {
  ...
  _addFrameChild(frameId, childId) {
    const frame = this.getObject(frameId);
    if (!frame || frame.type !== 'frame') return;
  ...
  _syncContainmentAfterMutation(id) {
    const obj = this.getObject(id);
    if (!obj || obj.type === 'connector') return;
    ...
    if ((nextParent?.id || null) !== (currentParentId || null)) {
      this._setObject({ ...obj, parentFrameId: nextParent ? nextParent.id : null });
    }
  }

  getAttachableAtPoint(wx, wy, excludeId = null) {
