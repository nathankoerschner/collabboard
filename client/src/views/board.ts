...
export const boardView: View = {
  async render(container, { boardId }) {
    ...
    const boardKeyHandler = (e: KeyboardEvent): void => {
    ...
    (window as unknown as Record<string, unknown>)._boardKeyHandler = boardKeyHandler;
  ...
  destroy() {
    const handler = (window as unknown as Record<string, unknown>)._boardKeyHandler as ((e: KeyboardEvent) => void) | undefined;
    if (handler) {
      window.removeEventListener('keydown', handler);
      (window as unknown as Record<string, unknown>)._boardKeyHandler = null;
    }
    if (zoomInterval) {
