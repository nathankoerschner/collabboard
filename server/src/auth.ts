let clerkVerifyToken: ((token: string, options: { secretKey: string }) => Promise<Record<string, unknown>>) | null = null;

async function loadClerk(): Promise<typeof clerkVerifyToken> {
  if (!process.env.CLERK_SECRET_KEY) return null;
  try {
    const clerk = await import('@clerk/backend');
    return clerk.verifyToken as typeof clerkVerifyToken;
  } catch {
