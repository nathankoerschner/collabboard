...
export async function migrate(): Promise<void> {
  ...
  // In dev (tsx): __dirname is server/src. In production (tsc): __dirname is server/dist/server/src.
  // Migrations always live at server/src/migrations.
  let migrationsDir = path.join(__dirname, 'migrations');
  if (!fs.existsSync(migrationsDir)) {
    // Compiled path fallback: walk up to find server/src/migrations
