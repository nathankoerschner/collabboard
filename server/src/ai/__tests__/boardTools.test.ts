...
describe('scoped read tools', () => {
  test('getObjectById returns one object', () => {
    const sticky = createSticky('single');
    const result = runner.getObjectById({ objectId: sticky.id }) as any;
    expect(result.found).toBe(true);
    expect(result.object.id).toBe(sticky.id);
  });

  test('listObjectsByType filters by type', () => {
    createSticky('a');
    createShape('rectangle');
    const result = runner.listObjectsByType({ type: 'sticky', limit: 10 }) as any;
    expect(result.count).toBe(1);
    expect(result.objects[0].type).toBe('sticky');
  });

  test('getObjectsInViewport returns intersecting objects only', () => {
    runner.createStickyNote({ text: 'near', x: 100, y: 100 });
    runner.createStickyNote({ text: 'far', x: 5000, y: 5000 });
    const result = runner.getObjectsInViewport({
      centerX: 100,
      centerY: 100,
      width: 400,
      height: 400,
    ...
    expect(result.count).toBe(1);
    expect(result.objects[0].text).toBe('near');
  });
});

describe('batch tools', () => {
  test('createObjectsBatch creates multiple objects', () => {
    const result = runner.createObjectsBatch({
      operations: [
        { toolName: 'createStickyNote', args: { text: 'a', x: 100, y: 100 } },
        { toolName: 'createShape', args: { type: 'rectangle', x: 200, y: 200 } },
      ],
    }) as any;

    expect(result.okCount).toBe(2);
    expect(result.errorCount).toBe(0);
    expect(runner.createdIds.size).toBe(2);
  });

  test('updateObjectsBatch updates multiple objects', () => {
    const sticky = runner.createStickyNote({ text: 'old', x: 10, y: 20 });
    const shape = runner.createShape({ type: 'rectangle', x: 0, y: 0 });

    const result = runner.updateObjectsBatch({
      operations: [
        { toolName: 'moveObject', args: { objectId: shape.id, x: 333, y: 444 } },
        { toolName: 'updateText', args: { objectId: sticky.id, newText: 'new' } },
      ],
    }) as any;

    expect(result.okCount).toBe(2);
    expect(runner.objects.get(shape.id)!.x).toBe(333);
    expect(runner.objects.get(sticky.id)!.text).toBe('new');
  });

  test('deleteObjectsBatch deletes multiple objects', () => {
    const a = createShape();
    const b = createSticky();
    const result = runner.deleteObjectsBatch({
      operations: [
        { toolName: 'deleteObject', args: { objectId: a.id } },
        { toolName: 'deleteObject', args: { objectId: b.id } },
      ],
    }) as any;
    expect(result.okCount).toBe(2);
    expect(runner.objects.has(a.id)).toBe(false);
    expect(runner.objects.has(b.id)).toBe(false);
  });
});

describe('createStructuredTemplate', () => {
  test('creates outer and section frames', () => {
    const result = runner.createStructuredTemplate({ template: 'swot', x: 0, y: 0 }) as any;
    expect(result.outerFrameId).toBeTruthy();
    expect(result.sectionFrameIds).toHaveLength(4);
    const outer = runner.objects.get(result.outerFrameId)!;
